...
namespace PurgeUselessPulledClouds
{
    class Program
    {
        static void Main(string[] args)
        {
            Directory.CreateDirectory("BestMatch");
            Directory.CreateDirectory("AlrightMatch");
            Directory.CreateDirectory("BadMatch");
            ...
            foreach (string file in files)
            {
                ...
                {
                    MoveAllFiles(file_PANOID, file_CLOUDID, "BestMatch");
                }
                else if (avgBrightness > 0.5 && /*pixelsWithZeroBr < ((imgDims.X * imgDims.Y) / 6)*/ lowestBrightness > 0.2)
                {
                    MoveAllFiles(file_PANOID, file_CLOUDID, "AlrightMatch");
                }
                else
                {
                    MoveAllFiles(file_PANOID, file_CLOUDID, "BadMatch");
                }
            }
        }

        static void MoveAllFiles(string panoID, string cloudID, string newFolderName)
        {
